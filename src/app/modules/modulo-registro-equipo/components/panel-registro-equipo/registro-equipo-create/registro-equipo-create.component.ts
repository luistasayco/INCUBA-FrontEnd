...
export class RegistroEquipoCreateComponent implements OnInit, OnDestroy {
  ...
  getOnRepuestoPorModelo() {
    ...
    if (codigoModelo !== '') {
      ...
      this.subscription$ = new Subscription();
      this.subscription$ = this.registroEquipoService
        .getRepuestoSeleccionados(vModelo)
        .subscribe((data: RepuestoPorModeloModel[]) => {
          this.listRespuestoPorModelo = [];
          this.listRespuestoPorModelo = [...data].filter(
            (xFila) =>
              xFila.flgAccesorio === false && xFila.flgPredeterminado === false
          );
        });
    }
  }
          ...
          (resp) => {
            if (resp) {
              ...
              this.mensajeGenerandoInformacion =
                'Generando Informacion - A.- Detalle de Mantenimiento...!!!';
  ...
  onLlenarInventarioRepuestoNoPredeterminado() {
    ...
    this.listClonedDetalle6Repuesto.forEach((repu) => {
      if (
        !this.nuevoRepuestosNoPredeterminado.find(
          (r) => r.value === repu.codigoRepuesto
        )
      ) {
        this.nuevoRepuestosNoPredeterminado.push({
          label: repu.codigoRepuesto + '-' + repu.descripcion,
          value: repu.codigoRepuesto,
        });
      }
  ...
  onToGrabar() {
    ...
    if (this.modeloItem.responsablePlanta === '') {
      this.mensajePrimeNgService.onToErrorMsg(
        this.globalConstants.msgExitoSummary,
        `Ingresar Responsable de Planta`
      );
      return;
    }
    ...
    this.subscription$ = this.registroEquipoService
      .setInsertTxRegistroEquipo(this.modeloItem)
      .subscribe(
        () => {
